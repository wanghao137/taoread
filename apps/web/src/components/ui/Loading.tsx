...
/** 加载态（design-system §7 + docs/13 P0-2）：白天小场景呼吸 + 吉祥物陪伴 + 正向文案 + aria-busy
 *
 * 不识字孩子看不懂「正在准备」这行字——吉祥物是给他们的信号：
...
export function Loading({ label = '小桃正在准备…' }: { label?: string }) {
  return (
    <div aria-busy="true" aria-live="polite" className="flex flex-col items-center gap-4 py-16">
      <motion.div
        aria-hidden
        className="h-16 w-16 overflow-hidden rounded-full border-ink border-2 shadow-card"
