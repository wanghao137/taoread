...
## 2026-09-17 视频重测 + 完成度审计（用户两问）

**问题一：视频重测——结论推翻重写。** 之前「45 分钟超时是免费档上限」的归因**是错的**。探针实证：建任务时云端偶发 `503 video_queue_full`（队列满），只要退避重试就能建成；建成之后 4 秒视频约 5 分钟渲染完成（pending → in_progress 10/70/90/100 → completed，mp4 4.3 MB）。**全链路验收通过**（probe-video-route.mjs）：HTTP 路由建任务 → 轮询 → 云端完成 → 服务端下载 mp4 回填 DB → `/api/media/` 直取 `content-type=video/mp4`。

- `createVideoTask` 加有界退避重试（5 次、12–60s 递增；400 参数错不重试）+ 3 单测
- 修 `.mp4` MIME 缺失（否则浏览器 `<video>` 拿到 octet-stream 拒播——视频唯一跑通那次就翻车）+ 6 单测

**问题二：完成度审计（子代理实证，非看文档）。** P0 声明属实，质量门数字一个不虚（367 单测 / 14 e2e / 32 张带标识插画全部实跑复核）。真正待办：
- 🔴 已在本轮清零：mp4 MIME、label-art.mjs 复现命令（改 `npx tsx` + `import.meta.url`，cwd 无关）
- 🟡 本轮打磨：夜灯墙 🕯️ emoji 换 SVG 灯；孩子端「无档案」死路给「回到登录页」出口；`/dev/kitchen-sink` 只在 DEV 挂路由；ShelfManager mp 死代码清除；schema status 注释对齐
- **P1 八项**（验收日后 1–2 轮）：首次引导 / 底部导航 / 继续读大卡片 / 桃树生长 / 周报兴趣分布 / 就寝调暗 / 庆祝音 / 音节拆分——七项完全未动，P1-3「继续读」只在旧选书流有半套（排序 + 🔖 前缀），不在桃书架

**质量门（复核）：** 378 单测（285 server + 93 web，+11）｜e2e 14/14（1.0 分钟）｜lint 0 error｜typecheck 通过｜key 扫描 0 命中。提交 75c5038。

---

## 2026-09-17 P1 第一批：首次引导 / 底部导航 / 继续读大卡片

回答第四轮第一性问题「4 岁孩子第一次被塞一台设备，有人教他吗」——之前答案是没有。

**P1-1 首次运行三步引导：** 月亮（今晚的故事在这开始）→ 封面（点书架挑书，不识字按小喇叭试听）→ 喇叭（每页都能听，睡着也没关系）。localStorage 一次性标记；隐私模式存储禁用时**静默降级为「已看过」**——不能因为存不下就卡死孩子。家长端设置页可清标记重播。+5 单测。

**P1-2 持久底部导航：** 之前孩子端只有顶部一个「🍑 桃书架」+「换家庭」，沉浸阅读时滑出视野，想回书架只能乱点。现在 SVG 图标三 tab（月亮/书架/夜灯）钉在底部，min-h-touch。夜灯角标只在有灯时亮（空态不诱导）；阅读器/收尾/庆祝等全屏流程不展示（Lillard 2011：减少与情节无关的视觉噪音）。

**P1-3 继续读大卡片：** 首屏最大的入口，进度最高且未读完的书，含封面+进度条+剩余章数+剩余朗读时长。只在无搜索/筛选时出现（孩子已在找特定书时不打扰）。未读书卡片副标题显示「约 N 分钟读完」。remainingMinutes：中文 150 字/分、英文 100 词/分，越界进度钳制。+7 单测。

**坑（本轮新踩）：**
- onboarding 最初只接在「单娃/已记忆孩子」路径，**多孩家庭选完孩子不弹**（演示家庭恰是双娃）——enterGate 那条路也要接
- 改卡片副标题时把「读到 N%」重复输出（封面进度条已有），触发 strict mode violation；有进度的书副标题只补章数
- e2e 每用例独立上下文 → localStorage 每次为空 → 引导每次都弹；loginAsChild 必须显式走完三步，否则后续断言全超时（8 个用例集体失败，根因是这一个）
- 一个用例中途失败会跳过它的会话收尾清理，污染后续用例的门屏（「今晚还没开始」断言失败）——失败传播链比想象的远

**质量门：** 390 单测（285 server + 105 web，+12）｜e2e 14/14（1.4 分钟）｜lint 0 error｜typecheck/audit 通过。提交 c0a71fb。
